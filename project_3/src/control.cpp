...
#define YAW_CHANGE_NUM					7
#define YAW_CROSS_NUM					2	
#define FINISHED_NUM					9
...
float fly_height[9] = { 2.0, 2.0, 2.0, 1.1, 1.2, 2.0, 2.0, 1.3, 2.0};
float cross_height[4] = {2.1, 2.3, 2.5, 2.0};
...
bool isArrived_xy_for_img(px4_autonomy::Position &pos, px4_autonomy::Position &pos_sp)
{
	...
	if(sqrt((pos_sp.x - pos.x)*(pos_sp.x - pos.x) + (pos_sp.y - pos.y)*(pos_sp.y - pos.y))< dd) 
	{
		return true;
	}
	else
...
int main(int argc, char **argv)
{
	...
	Reletive_pos<<
   -3.3,  0.0,  0,   TARGET_LANDING,      	//1-2Parking
   -4.8,  0.0,  0,   TARGET_LANDING, 	    //2-3Cross  //-3.3-1.5
	2.3,  2.28,  0,   TARGET_LANDING,		//3-4P    //0.87 + 1.5  2.28
    3.0,  2.27,  0,   TARGET_CROSS_CIRCLE,	//4-5C
    2.35, 2.49,  0,   TARGET_CROSS_CIRCLE,	//5-6C
   -1.38, 2.62,  0,   TARGET_LANDING,		//6-7P
   -2.38, 1.86,  0,   TARGET_LANDING,		//7-8P
   -1.9,  -2.5, 0,   TARGET_CROSS_CIRCLE,	//8-9C
   -1.33, -3.26, 0,   TARGET_LANDING;		//9-10P
