...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber lidar_sub = nh.subscribe("/mavros/lidar/range", 1, lidarCallback);
    ...
    while(nh.ok())
    {
        ...
        pose_cal.z = pos(2);
        pose_cal.yaw = yaw;
        if(pose_cal.yaw > 3.14) pose_cal.yaw = yaw - 6.283;
        else if(pose_cal.yaw < -3.14) pose_cal.yaw = yaw + 6.283;
            ...
            switch(status)
            {
                ...
                case 1:  //wait on ground in offboard mode
                {
                    ...
                    if(pos(2) < land_height + 0.2)
                        cmd_pose.pose.position.z = pos(2) - 2.0;
                ...
                case 3: //land process
                {
                    ...
                    if(dec_height > -0.15f) dec_height = -0.15f;
